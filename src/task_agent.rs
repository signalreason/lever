...
pub fn run_task_agent(
    ...
    for _ in 0..3 {
        ...
        if let Some(delay) = rate_limit_retry_delay(&codex_log_abs)? {
            if delay > 0 {
                eprintln!(
                    "Rate limit retry: sleeping {}s before retry.",
                    delay
                );
...
fn rate_limit_retry_delay(log_path: &Path) -> Result<Option<u64>, DynError> {
    ...
    if File::open(log_path).and_then(|mut f| f.read_to_string(&mut raw)).is_err() {
        return Ok(None);
    }
...
fn reset_task_attempts(
    ...
    obs.insert("last_update_utc".to_string(), Value::from(utc_timestamp("%Y-%m-%dT%H:%M:%SZ")?));
    if !note.is_empty() {
...
fn ensure_observability(task: &mut Map<String, Value>) -> &mut Map<String, Value> {
    if !task.get("observability").map(|value| value.is_object()).unwrap_or(false) {
