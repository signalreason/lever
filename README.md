...
- `--context-compile` enables context compilation for each run and `--no-context-compile` disables it; `--context-failure-policy <best-effort|required>` selects how failures are handled (default: best-effort).
