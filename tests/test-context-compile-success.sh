...
cat > "$repo_dir/prd.json" <<'JSON'
{
  "tasks": [
    {
      "task_id": "T1",
      "title": "Context compile success",
      "status": "unstarted",
      "model": "gpt-5.1-codex-mini",
      "definition_of_done": [
        "Write compiled context pack"
      ],
      "recommended": {
        "approach": "Use assembly stub"
      }
    }
  ]
}
...
if [[ ! -d "$pack_dir" ]]; then
  echo "Expected pack directory at $pack_dir" >&2
  exit 1
fi

pack_rel="${pack_dir#"$repo_dir"/}"
if [[ "$pack_rel" != .ralph/runs/T1/*/pack ]]; then
  echo "Expected pack path under .ralph/runs/T1/<run_id>/pack, got: $pack_rel" >&2
  exit 1
fi
...
for file in "${required[@]}"; do
  if [[ ! -f "$pack_dir/$file" ]]; then
    echo "Expected pack file $file at $pack_dir" >&2
    exit 1
  fi
done
...
Task title: Context compile success
...
{"definition_of_done":["Write compiled context pack"],"model":"gpt-5.1-codex-mini","recommended":{"approach":"Use assembly stub"},"status":"unstarted","task_id":"T1","title":"Context compile success"}
