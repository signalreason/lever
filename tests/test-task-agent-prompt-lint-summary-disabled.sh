...
cat > "$repo_dir/prd.json" <<'JSON'
{
  "tasks": [
    {
      "task_id": "T1",
      "title": "Prompt lint summary disabled",
      "status": "unstarted",
      "model": "gpt-5.1-codex-mini",
      "definition_of_done": [
        "Do not include lint summary"
      ],
      "recommended": {
        "approach": "Skip lint summary"
      }
    }
  ]
}
...
Task title: Prompt lint summary disabled
...
{"definition_of_done":["Do not include lint summary"],"model":"gpt-5.1-codex-mini","recommended":{"approach":"Skip lint summary"},"status":"unstarted","task_id":"T1","title":"Prompt lint summary disabled"}
...
if grep -q "Lint summary" "$prompt_file"; then
  echo "Expected lint summary to be absent when disabled" >&2
  exit 1
fi
